...
export const getSystemPath = (path: string = '', onlyFolder: boolean = false) => {
  return axios.get<Response<DataNode[]>>(`afm/file/path`, {
    params: {
      path,
      onlyFolder,
    },
...
export const createFile = (path: string, type: FolderType) => {
  return axios.get<Response<DataNode[]>>(`afm/file/createFile`, {
    params: {
      path,
      type,
    },
...
export const deleteFiles = (path: string[]) => {
  return axios.delete<Response>(`afm/file/deleteFile`, {
    data: `${path.join(',')}`,
  });
};

export const renameFile = (oldFileName: string, newFileName: string) => {
  return axios.put<Response>(`afm/file/renameFile`, {
    oldFileName,
    newFileName,
  });
};
...
export const clonedFiles = (targetPath: string, files: string[]) => {
  return axios.post<Response>(`afm/file/cloneFile`, {
    targetPath,
    files: files.join(','),
  });
};
...
export const fileAttr = (path: string) => {
  return axios.get<Response<FileAttr>>(`afm/file/fileAttr`, {
    params: {
      path,
    },
  });
