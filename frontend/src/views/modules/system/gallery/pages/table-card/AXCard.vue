<template>
    ...
    <ul class="table_card_list flex-1" ref="cardRef">
      <li
        v-for="(item, key) in galleryCardData"
        :key="key"
        size="small"
        @click="selectSystemGallery(item)"
        :class="{ active: galleryKeys.includes(item.id) }"
        @dblclick="galleryEdit(item.id)"
        :draggable="true"
      ...
      >
        <a-card>
          <div class="active" v-if="galleryKeys.includes(item.id)">{{ key + 1 }}</div>
...
import { dragstart, drop, selectSystemGallery, galleryCardData } from '../../data/card';
import { galleryEdit } from '../../data/curd';
import { galleryKeys } from '../../data/table';
...
nextTick(() => {
  useSortable(cardRef, galleryCardData.value, {
    animation: 200,
