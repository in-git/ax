  ...
  <a-card class="table__card" :bordered="false" :body-style="{ padding: '0' }">
    <ul class="table_card_list flex-1" ref="cardRef">
      <li
        ...
        :key="key"
        size="small"
        @click="selectSystemRole(item)"
        :class="{ active: roleKeys.includes(item.id) }"
        @dblclick="selectRole(item.id)"
        :draggable="true"
      ...
      >
        <a-card>
          <div class="active" v-if="roleKeys.includes(item.id)">{{ key + 1 }}</div>
...
import { dragstart, drop, roleCardData, selectSystemRole } from '../../data/card';
import { selectRole } from '../../data/curd';
import { roleKeys } from '../../data/table';
...
nextTick(() => {
  useSortable(cardRef, roleCardData.value, {
    animation: 200,
