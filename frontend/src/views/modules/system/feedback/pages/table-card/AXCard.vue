<template>
    ...
    <ul class="table_card_list flex-1" ref="cardRef">
      <li
        ...
        :key="key"
        size="small"
        @click="selectSystemFeedback(item)"
        :class="{ active: feedbackKeys.includes(item.id) }"
        @dblclick="feedbackEdit(item.id)"
        :draggable="true"
      ...
      >
        <a-card>
          <div class="active" v-if="feedbackKeys.includes(item.id)">{{ key + 1 }}</div>
